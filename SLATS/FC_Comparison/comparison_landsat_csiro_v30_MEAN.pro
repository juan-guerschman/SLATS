pro comparison_Landsat_CSIRO_v30_mean
  compile_opt idl2
...
;  for m=0,3 do begin
    ...
    fname_plot = 'Comparison.Landsat-CSIRO_v30.MEAN.ps'
    ...
    CSIRO_V30_BS = READ_BINARY(fname, DATA_DIMS=DIMS, Data_Type=4)
          ...
          DIFF_PV = Landsat_PV-CSIRO_v30_PV
          DIFF_NPV = Landsat_NPV-CSIRO_v30_NPV
          DIFF_BS = Landsat_BS-CSIRO_v30_BS
        ...
        PS_Start, fname_plot
          ...
          yrange = [Min(yy), Max(yy)]          
            ...
            density = HIST_ND(TRANSPOSE([[Landsat_PV[where_both_ok]],[CSIRO_v30_PV[where_both_ok]]]), MIN=0,MAX=1,NBINS=100)         
  ...
  ;             /fit;, OOB_Low='grey';, Position=[0.2, 0.7, 0.8, 0.85];, OOB_Low='white'
            ...
            density = HIST_ND(TRANSPOSE([[Landsat_NPV[where_both_ok]],[CSIRO_v30_NPV[where_both_ok]]]), MIN=0,MAX=1,NBINS=100)         
            cgImage, Density, XRange=xrange, YRange=yrange, /Axes, Palette=palette, $
  ...
  ;             /fit;, OOB_Low='grey';, Position=[0.2, 0.7, 0.8, 0.85];, OOB_Low='white'
            ...
            density = HIST_ND(TRANSPOSE([[Landsat_BS[where_both_ok]],[CSIRO_v30_BS[where_both_ok]]]), MIN=0,MAX=1,NBINS=100)         
            cgImage, Density, XRange=xrange, YRange=yrange, /Axes, Palette=palette, $
